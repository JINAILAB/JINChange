...
def change_spatial(tensor, b1=2):
    return rearrange(tensor, 'b c (h1 h2) (w1 w2) -> b c (h1 w2) (w1 h2)', h2=b1, w2=b1)

def change_channel(tensor1, tensor2):
    _, c, _, _ = tensor1.size()
    tensor1_1 = tensor1[:, :c//2, :, :]
    tensor1_2 = tensor1[:, c//2:, :, :]
    
    tensor2_1 = tensor2[:, :c//2, :, :]
    tensor2_2 = tensor2[:, c//2:, :, :]
    
    out1 = torch.cat([tensor1_1,tensor2_1], dim=1)
    out2 = torch.cat([tensor1_2,tensor2_2], dim=1)
